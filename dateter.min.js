...
b("#Shadow").remove();a.show()}}).hide();b("body").append(Shadow);b(".dateterPopup").hide();b("#"+a.data("Settings").uniqueName,{css:{"z-index":"1000000"}}).show();b("#Shadow").show()});b("body").append(calGlobal=b("<div/>",{"class":a.data("Settings").borderClass+" "+a.data("Settings").borderRoundClass+" "+a.data("Settings").backgroundClass+" "+a.data("Settings").shadeClass+" dateterPopup",id:a.data("Settings").uniqueName}).hide());calGlobal.append(calBox=b("<div/>"));b.fn.dateter.drawCalendar(calBox,
a.data("Settings"),a);heightAdjust=b(a).height();heightAdjust=0}};b.fn.dateter.moveMonth=function(a,d,c,e,f){c.data("Settings",b.extend(true,c.data("Settings"),d));localsettings=c.data("Settings");a.click(function(){l=false;c.data("Settings").monthsMoved=parseInt(c.data("Settings").monthsMoved+e);if(c.data("Settings").startYear==-1){var g=Date.today().add(c.data("Settings").monthsMoved).months().toString("M"),i=Date.today().add(c.data("Settings").monthsMoved).months().toString("yyyy"),h=b.fn.keyInArray(Date.parse(parseInt(g)+
...
6;realCellHeight--;e=localSettings.height}a.append(e=b('<table cellpadding="0" cellspacing="0" border="0"/>').css({width:localSettings.width,height:e}).addClass(localSettings.borderClass).attr({id:"calBox"+localSettings.uniqueName}));e.empty();for(var g=0;g<6;g++){e.append(b("<tr/>",{id:localSettings.uniqueName+"w"+g}));for(var i=0;i<7;i++){b("#"+localSettings.uniqueName+"w"+g).append(b("<td/>",{css:{fontSize:"11px",textAlign:"center",width:localSettings.cellWidth,height:localSettings.cellHeight},
...
true)curDay.html(b("<div/>",{id:"dayBox"+f,css:{overflow:"hidden",height:"100%",width:"100%"},html:clickArea=b("<div/>",{css:{borderTop:0,borderLeft:0,borderRight:0,width:"100%",height:"15px"},"class":localSettings.borderClass+" "+localSettings.shadeClass+" "+localSettings.fontColor,html:f})}).append(clickArea2=b("<div/>",{id:"eventBox"+f,css:{borderTop:0,borderLeft:0,borderRight:0,overFlow:"hidden",width:"100%",height:parseInt(realCellHeight-15)},"class":"clickarea2"})));else clickArea=curDay.css({height:realCellHeight}).addClass(localSettings.fontColor).html(f);
if(localSettings.pastDayShades)curDay.addClass(h[1]||h[2]||h[3]?localSettings.pastDayShadeClass:localSettings.calendarCell).css({cursor:"pointer"});localSettings.highLightToday&&Date.today().toString("d")==f&&Date.today().toString("M")==localsettings.month&&Date.today().toString("yyyy")==localsettings.year&&curDay.addClass(localSettings.highLightTodayClass).removeClass(localSettings.pastDayShadeClass).css({cursor:"pointer"});!localSettings.pastDayShades&&!(Date.today().toString("d")==f&&Date.today().toString("M")==
localsettings.month&&Date.today().toString("yyyy")==localsettings.year)&&curDay.addClass(localSettings.calendarCell);clickArea.click(function(){if(a.data("Settings").callbackFn){b("#Shadow").remove();a.data("Settings").callbackFn(localSettings.month,b(this).text(),localSettings.year);if(a.data("Settings").noClick===false){c.show();a.parent().fadeOut(300)}}})}else{curDay=b("#"+localSettings.uniqueName+"d"+d);localSettings.largeDisplay===true?curDay.html(b("<div/>",{id:"dayBox"+f,css:{overflow:"hidden",
height:"100%",width:"100%"}}).html(clickArea=b("<div/>",{html:b("<font/>",{"class":localSettings.fontColor,html:"&nbsp;"}),css:{borderTop:0,borderLeft:0,borderRight:0,width:"100%",height:"16px"}})).append(clickArea2=b("<div/>",{id:"eventBox"+f,css:{borderTop:0,borderLeft:0,borderRight:0,overFlow:"hidden",width:"100%",height:parseInt(realCellHeight-15)},"class":"clickarea2"}))):curDay.html(b("<div/>",{css:{height:realCellHeight},html:"&nbsp;"}))}d++}}b.fn.dateter.drawHighlight(localSettings);if(a.data("Settings").monthSwitchFn)if(a.data("Settings").initialFetchMonths==
0)a.data("Settings").monthSwitchFn(localSettings.month,-1,localSettings.year);else for(v=1;v<parseInt(a.data("Settings").initialFetchMonths);v++){d=Date.today().add(a.data("Settings").monthsMoved-v).months().toString("yyyy");f=Date.today().add(a.data("Settings").monthsMoved-v).months().toString("M");e=Date.today().add(a.data("Settings").monthsMoved+v).months().toString("yyyy");g=Date.today().add(a.data("Settings").monthsMoved+v).months().toString("M");a.data("Settings").monthSwitchFn(g,-1,e);a.data("Settings").monthSwitchFn(f,
-1,d)}}})(jQuery);
